...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//set records shown in one page to 10
		if err != nil || recordPerPage < 1 {
			recordPerPage = 10
		}

		page, err := strconv.Atoi(c.Query("page"))
		//if a request contains page no of 0 or negative, just return the first page
		if err != nil || page < 1 {
			page = 1
		}
		...
		groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{"_id", "null"}}}, {"total_cnt", bson.D{{"$sum", 1}}}, {"data", bson.D{{"$push", "$$ROOT"}}}}}}
		projectStage := bson.D{
			{
				"$project", bson.D{
					{"_id", 0},
...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, insertionErr := foodCollection.InsertOne(ctx, food)
		if insertionErr != nil {
